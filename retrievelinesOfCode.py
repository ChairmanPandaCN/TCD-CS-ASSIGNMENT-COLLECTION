...
tk = "695193244278396dc5a04427f083a0a3a1420e87"

user_stats = {"dummy": {"additions": 0, "deletions": 0, "total": 0}}
# query github api for last year's commits
payload = {'since': '2019-01-01T00:00:00Z',
           'until': '2020-01-01T00:00:00Z', 'access_token': tk}
...
def collect_stats(commit_list):
    for m in commit_list:
        ...
        #commit = r2.json()
        # print commit
        git_diff_command = "git diff --shortstat " + \
            m['sha'] + " " + m['sha'] + "^"
        ...
        if(user in user_stats):
            ...
            new_stat = {'additions': ins_data,
                        'deletions': del_data, 'total': ins_data+del_data}
            user_stats[user] = new_stat


r = requests.get(
    "https://api.github.com/repos/996icu/996.ICU/commits", params=payload)
