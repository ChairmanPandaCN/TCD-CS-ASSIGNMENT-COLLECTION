...
add2(X,p((-A)+B),Z) :- minus(A,D),add2(D,B,C),add2(p(0),C,E),add2(X,E,Z).
add2(X,p(A+(-B)),Z) :- minus(B,D),add2(A,D,C),add2(p(0),C,E),add2(X,E,Z).
add2(p((-X)+Y),A,Z) :- minus(X,D),add2(D,Y,C),add2(p(0),C,E),add2(E,A,Z).
add2(p(X+(-Y)),A,Z) :- minus(Y,D),add2(X,D,C),add2(p(0),C,E),add2(E,A,Z).
add2(p((-X)+Y),p(A+B),Z) :- minus(X,D),add2(p(0),D+Y,E),add2(p(0),A+B,F),add2(E,F,Z).
