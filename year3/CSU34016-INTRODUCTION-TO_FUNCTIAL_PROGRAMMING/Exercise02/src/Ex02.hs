...
eval d (Add x y) =simplify d (+) x y
eval d (Sub x y) =simplify d (-) x y
eval d (Mul x y) =simplify d (*) x y
...
simplify d operator x y =case (eval d x,eval d y) of 
...
law4 (Mul (Add x w) (Sub y z)) = Just (Sub (Mul x y) (Mul w z))
law4 _                         = Nothing